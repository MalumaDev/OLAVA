...
def process_single_sample(data):
    ...
    shots = (None if 'shots' not in data else [process_single_sample(x) for x in data['shots']])

    if len(o_imgs_paths) > 1:  # multiple images in options, used for random selection
        return {'id': data['id'], 'question': question, 'options': data['options'], 'answer': data['answer'],
             'image': None, 'question_type': data['question_type'], 'shots': shots}
    else:
        return {'id': data['id'], 'question': question, 'options': data['options'], 'answer': data['answer'],
             'image': data['image_1'], 'question_type': data['question_type'], 'shots': shots}
...
def construct_prompt(sample, config):
    ...
    if ('shots' in sample) and (sample['shots'] is not None):
        _shots = []
        for s in sample['shots']:
            s = construct_prompt(s, config)
            _shots.append(s)
        sample['shots'] = _shots
